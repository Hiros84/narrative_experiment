...
// 数値らしい文字列（カンマ・空白・通貨記号付き）を数値に変換するユーティリティ
function parseLocaleNumber(v) {
    if (typeof v === 'number') return v;
    if (typeof v === 'string') {
        // カンマや空白、円記号を除去
        const cleaned = v.replace(/[\s,]/g, '').replace(/[￥¥円]/g, '');
...
function updateAnalyses() {
    ...
    // ユーザーが「量的」を選んだ列は、ここで文字列→数値に変換する（反映ボタン後に実行）
    const columnsMarkedNumerical = includedColumns.filter(key => scaleConfig[key] === 'numerical');
    columnsMarkedNumerical.forEach(col => {
