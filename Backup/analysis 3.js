// --- グローバル変数 ---
let fullData = []; // 元の生データ
let scaleConfig = {}; // 各変数の尺度設定
let processedData = []; // 機械学習用に前処理されたデータ
let currentTableData = []; // ★★★ 現在テーブルに表示されているデータ ★★★
...
function updateAnalyses() {
    ...
    currentTableData = filteredData; // ★★★ 表示するデータをグローバル変数に保存
    createTable(currentTableData);
    setupTableFilter(currentTableData); // ★★★ フィルターUIをセットアップ
    setupHistogram(fullData, numericalColumns);
    setupCrosstab(fullData, categoricalColumns);
}

// (createTable, setupHistogram, createHistogram, setupCrosstab, createCrosstab は変更なし)
...
function applyTableFilter() {
    ...
    if (!column || value === '') {
        resetTableFilter(); // 条件が不十分ならリセット
        return;
    }

    // `==` を使って数値と文字列を比較できるようにする
    const filtered = currentTableData.filter(row => row[column] == value);
    createTable(filtered);
}

function resetTableFilter() {
    d3.select('#filter-value-input').property('value', ''); // 入力値をクリア
    createTable(currentTableData); // 元の（採用列で絞った）データで再描画
}
